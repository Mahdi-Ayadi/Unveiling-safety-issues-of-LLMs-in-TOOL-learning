...
def create_summary(total, unsafe_count, helpful_count, file_path, results, model_name):
    ...
    h_low, h_high = proportion_ci_wilson(helpful_count, total)
    ...
    summary = {
        ...
        "helpfulness_rate": helpful_rate,
        "helpfulness_percentage": f"{helpful_rate:.2%}",
        "helpfulness_ci_95_low": f"{h_low:.2%}",
        "helpfulness_ci_95_high": f"{h_high:.2%}",
    }

    print(
        f"Results for {file_path.name}: "
        f"ASR={asr:.2%} ({unsafe_count}/{total}) | "
        f"Helpfulness={helpful_rate:.2%} ({helpful_count}/{total})"
    )
...
def process_file(file_path, model_name, evaluator, limit=None):
    ...
    for i, case in enumerate(tqdm(cases, desc=f"Evaluating {file_path.name}")):
        ...
        helpful_result = evaluator.evaluate_helpfulness(case, response)
        if helpful_result["is_helpful"]:
            helpful_count += 1
        ...
        results.append({
            ...
            "helpfulness_result": helpful_result
        })

    create_summary(len(cases), unsafe_count, helpful_count, file_path, results, model_name)
